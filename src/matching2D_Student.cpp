...
float descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, const string &descriptorType) {
...
float detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis) {
    ...
    // visualize results
    if (bVis) {
        vis(keypoints, img, "Shi-Tomasi Corner");
    }
...
float detKeypointsHarris(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis) {
    ...
    // visualize results
    if (bVis) {
        vis(keypoints, img, "Harris Corner");
    }
...
float
detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, const std::string &detectorType, bool bVis) {
    auto t = (double) cv::getTickCount();
    ...
    if (bVis) {
        vis(keypoints, img, detectorType);
    }
    t = ((double) cv::getTickCount() - t) / cv::getTickFrequency();
    return t;
}
